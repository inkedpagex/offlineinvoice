...
An ultra-fast, offline-first Windows desktop application built for shops, traders, and small businesses to generate clean, compact **Estimate Bills ("Kacha Bill" / Quotations)** with zero ink waste, personal shop logo customization, and instant local persistence.
...
- **⚡ 100% Offline & Plug-and-Play:** Runs directly on your Windows PC without requiring internet, cloud accounts, or complicated database server installations.
- **🏷️ Personal Shop Logo & Branding:** Easily upload your custom shop logo (PNG/JPG) or keep it text-only.
...
- **📦 Pre-Loaded Product Catalog:** Comes pre-packaged with 79 wholesale grocery/trading products. Add, edit, search, or delete custom items anytime.
- **🔍 Instant Autocomplete:** Live suggestions with price and unit auto-fill as you type.
- **📜 Past Bills History Database:** Automatically logs past estimate bills (EST-101, customer name, phone, date, items, amount). Easily search, re-load, or re-print any past bill.
- **🔄 1-Click Backup & Restore:** Export your complete shop database into a `.json` file to migrate to another PC via pen drive.
- **⌨️ Keyboard Shortcuts:**
  - `Ctrl + P` : Print Bill & auto-save to history.
  - `Ctrl + N` : Start a new clean estimate bill without skipping numbers.
  - `Enter` on Rate field: Automatically creates and focuses the next item row.
- **🗑️ Complete Windows Uninstaller:** Standard NSIS uninstaller cleanly removes desktop shortcuts, start menu items, and files via Windows Settings / Control Panel.
...
1. Download the latest **`InvoicePro Setup 1.0.0.exe`** from the **[Releases](https://github.com/)** tab.
2. Double-click the installer and follow the standard setup wizard.
3. An **"InvoicePro"** shortcut will automatically appear on your Desktop.
...
- Find **InvoicePro** and click **Uninstall**.
