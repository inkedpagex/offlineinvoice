    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 no-print transition-all">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          {/* Customer Details & Date Typography / Font Size Customization */}
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
            <div className="flex items-center justify-between mb-1">
              <label className="block text-xs font-bold text-slate-800 uppercase tracking-wider">
                Print & Header Font Sizes (कस्टम फॉन्ट साइज़)
              </label>
            ...
            <p className="text-[11px] text-slate-500 mb-3">
              Set your preferred font size for Party Name, Address, and Date on printouts and billing screen.
            </p>

            {/* Name Font Size Selector */}
            <div className="mb-3">
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                Customer / Party Name Font Size (नाम का साइज़)
              </label>
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  <button
                    key={item.key}
                    type="button"
                    onClick={() => setFormData({ ...formData, customerNameFontSize: item.key as any })}
                    className={`py-1.5 px-2 rounded-lg border text-xs font-bold transition-all text-center ${
                      (formData.customerNameFontSize || 'xl') === item.key
            ...
            {/* Address Font Size Selector */}
            <div className="mb-3">
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                Customer Address Font Size (पते का साइज़)
              </label>
              <div className="grid grid-cols-4 gap-2">
                {[
                  { key: 'normal', label: 'Normal' },
                  ...
                  <button
                    key={item.key}
                    type="button"
                    onClick={() => setFormData({ ...formData, customerAddressFontSize: item.key as any })}
                    className={`py-1.5 px-2 rounded-lg border text-xs font-bold transition-all text-center ${
                      (formData.customerAddressFontSize || 'xl') === item.key
                        ? 'border-sky-500 bg-sky-50 text-sky-800 ring-2 ring-sky-500/20 shadow-xs'
                        : 'border-slate-200 bg-white hover:bg-slate-100 text-slate-700'
                    }`}
                  >
